...
> **요약**: 변동성을 계산하는 윈도우 크기(10, 20, 40일)에 따른 민감도를 테스트함. **변동성은 종가 자체가 아니라 일별 수익률(%)의 표준편차로 계산**합니다 — 종가 기준으로 계산하면 가격 수준이 높은 시기에 등락폭이 같아도 표준편차가 구조적으로 커 보이는 편향이 있기 때문입니다(교차검증 과정에서 발견하여 계산 방식을 수정함). 10일 선(Cyan)은 단기 이벤트에 민감하게 반응하여 노이즈가 크고, 40일 선(Magenta)은 반응이 지연됨. **20일 선(Purple)이 추세적 변동성을 파악하는 데 가장 적합(Robust)함**. 2024년 하반기에 뚜렷한 변동성 급등이 **두 차례** 있었음 — 8/16(20일 수익률 표준편차 약 3.19%p)과 11/18(약 3.21%p, 관측 기간 중 최댓값) — 둘 다 평시(평균 약 1.63%p) 대비 약 2배 수준.
...
2. **변동성 극대화 및 거래량 동반 패닉 셀링**
   - **관찰(Fact)**: 변동성 그래프(3번, 일별 수익률 기준 20일 표준편차)에서 2024년 하반기에 **두 차례** 뚜렷한 급등이 있었음 — **8/16(약 3.19%p)**과 **11/18(약 3.21%p, 관측 기간 중 최댓값)**, 둘 다 평시(평균 약 1.63%p) 대비 약 2배 수준. 거래량 기준으로는 이 중 8/5가 연중 2위(약 5,461만 주)를 기록했으나, 연중 최대 거래량 자체는 이 구간이 아닌 **2024-01-11(약 5,769만 주)**이었음.
   - **원인(Why)**: 주요 지지선 붕괴에 따른 기관 및 외국인의 대규모 패닉 셀링(투매).
   - **행동(Action)**: 20일 표준편차가 과거 6개월 평균치의 **2.5배를 초과하며 거래량이 1.5배 이상 터지는 하락 구간**에서는 신규 매수를 전면 보류하고 변동성이 1.5배 이하로 안정화될 때까지 대기(Wait & See). (※ 실측 검증 결과, 이 2년치 데이터에서 20일 표준편차가 6개월 평균 대비 도달한 최댓값은 약 **1.57배**(11/18 기준)로, "2.5배" 문턱은 이 기간 동안 한 번도 도달한 적이 없는 보수적인 기준이었습니다. 실전 적용 전 임계값 재보정이 필요합니다.)
